...
def parse_kickoff(start_time_str: str) -> datetime | None:
    ...
    if not start_time_str:
        return None
    try:
        # Chuẩn hoá: bỏ 'Z' cuối hoặc offset
...
def calc_is_live(computed_status: str, is_live: bool) -> bool:
    return is_live or computed_status == "live"
...
BASE_URL = "https://quechoa10.live"

# API tRPC — lấy toàn bộ danh sách trận kèm stream URL
API_MATCHES = (
    "https://quechoa10.live/api/trpc/public.match.list"
    "?batch=1&input=%7B%220%22%3A%7B%22json%22%3A%7B%22limit%22%3A100%7D%7D%7D"
)
...
# Mapping sport slug → nhãn hiển thị
# Dựa trên danh sách môn của quechoa10.live
SPORT_MAP = {
    "bong-da":    "⚽ Bóng Đá",
...
def fetch_matches() -> list:
    """Gọi API tRPC lấy danh sách trận, trả về list item thô."""
    try:
        t   = int(time.time() * 1000)
        url = f"{API_MATCHES}&_t={t}"
        res = requests.get(url, headers=HEADERS, timeout=15)
        data = res.json()
        # tRPC trả về list, phần tử đầu có result.data.json
        if isinstance(data, list) and len(data) > 0:
            return data[0].get("result", {}).get("data", {}).get("json", [])
        return []
    except Exception as e:
        print(f"  Loi fetch matches: {e}")
        return []
...
def get_blv_names(commentators: list) -> str:
    """Lấy tên BLV, ưu tiên isPrimary=True lên đầu."""
    primary   = [c["account"]["name"] for c in commentators if c.get("isPrimary")]
    secondary = [c["account"]["name"] for c in commentators if not c.get("isPrimary")]
    return ", ".join(primary + secondary)


def get_stream_url(commentators: list) -> str | None:
    """
    Lấy stream URL từ BLV isPrimary=True, ưu tiên label FHD.
    Nếu không có primary thì lấy BLV đầu tiên.
    """
    targets = [c for c in commentators if c.get("isPrimary")] or commentators
    for c in targets:
        stream_urls = c.get("account", {}).get("streamUrls", [])
        # Ưu tiên FHD
        for s in stream_urls:
            if s.get("label", "").upper() == "FHD" and s.get("url"):
                return s["url"]
        # Fallback: lấy URL đầu tiên có giá trị
        for s in stream_urls:
            if s.get("url"):
                return s["url"]
    return None
...
def get_matches() -> list:
    raw_items = fetch_matches()
    matches   = []

    for item in raw_items:
        slug           = item.get("slug", "")
        is_live        = item.get("isLive", False)
        computed_status= item.get("computedStatus", "upcoming")
        ...
        # Bỏ qua trận đã kết thúc hoàn toàn (FT) và không live
        metadata    = item.get("metadata") or {}
        api_status  = metadata.get("apiStatus", "")
        if api_status == "FT" and not is_live and computed_status != "live":
            continue

        # Bỏ qua trận không có BLV
        if not commentators:
            continue

        kickoff_dt = parse_kickoff(start_time_str)

        # Lọc theo thời gian
        if not is_within_48h(kickoff_dt):
            continue

        sport_slug = sport.get("slug", "unknown")
        team_a     = home_team.get("name", "")
        team_b     = away_team.get("name", "")
        name       = f"{team_a} vs {team_b}" if team_a and team_b else slug[:50]
        ...
        matches.append({
            ...
            "league_icon": tournament.get("iconUrl", ""),
            "card_bg":     sport.get("matchCardBgUrl", ""),
            "blv":         get_blv_names(commentators),
            "stream_url":  get_stream_url(commentators),
            "is_live":     calc_is_live(computed_status, is_live),
            "is_hot":      is_hot,
        })
...
def build_channel(match: dict, thumb_url: str = "") -> dict:
    ...
    if stream_url:
        ...
        stream_links.append({
            ...
            "url":     stream_url,
            "request_headers": [
                {"key": "Referer",    "value": "https://quechoa10.live/"},
                {"key": "Origin",     "value": "https://quechoa10.live"},
    ...
    channel = {
        ...
        "sources": [{
            ...
            "contents": [{
                "id":   ct_id,
                "name": match["name"],
                "streams": [{"id": st_id, "name": "QC", "stream_links": stream_links}],
            }],
        }],
        "org_metadata": {
...
def main():
    ...
    print("Lay danh sach tran tu QuechoaTV API...")
    ...
    output = {
        "id":          "quechoa",
        "url":         "https://quechoa10.live",
        "name":        "QuechoaTV",
        "color":       "#83ff65",
        "grid_number": 3,
        "image":       {"type": "cover", "url": "https://quechoa10.live/quechoatv-logo-v2.png"},
        "groups":      groups,
    }
